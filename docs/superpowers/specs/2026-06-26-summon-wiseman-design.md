# `/summon-wiseman` 설계 문서

- 작성일: 2026-06-26
- 상태: 설계 확정 대기 (사용자 리뷰)
- 제품명: **Wiseman** — "코딩하기 전에 여쭤보는 현자(wise man)"
- 한 줄 요약: 현재 프로젝트를 분석·심층 조사하여, **그 프로젝트에 딱 맞는 "현자(Wiseman)"** — 즉 프로젝트 전용 지식이 담긴 MCP 서버 — 를 만들어주는 Claude Code 플러그인 + 스킬.

> 네이밍 노트: `wiseman`은 확정 제품명이다. 현재 저장소 디렉토리명은 `teachers-mcp`(가명)이며, 추후 `wiseman` 계열로 리네임 가능(코스메틱, MVP 범위 밖).

---

## 1. 목적과 핵심 가치 기준

Wiseman은 **"우리 프로젝트에 딱 맞는 LLM 위키"** 를 만든다. 코딩 중 Claude가 행동 전에 "현자에게 여쭤보면", 그 프로젝트의 실제 스택·버전·관례·함정에 맞는 답을 받는다.

### 절대적 성공 기준 (Non-negotiable)

> **현자에게 물어서 얻은 답은, 모델이 자기 내부 지식만으로 짜는 것보다 반드시 더 나아야 한다.**

둘이 다를 게 없으면 Wiseman을 만들 이유가 없다. 따라서 저장되는 모든 지식은 다음을 만족해야 한다:
...
`wiseman` 플러그인은 다음을 번들한다:

- `summon-wiseman` **스킬** — 분석·조사·DB 생성 지시문 (빌드 시점)
- **범용 Wiseman MCP 서버** — DB를 읽어 검색해주는 빈 껍데기 (질의 시점)
- (향후) 편집 대시보드, 임베딩 인덱서
  ...
  "mcpServers": {
    "wiseman": {
      ...
      "args": [
        ...
        "wiseman-mcp",
        "--db", "${CLAUDE_PROJECT_DIR}/.wiseman/wiseman.db"
      ]
    }
  }
}
...
`/summon-wiseman` 실행 시 **프로젝트에 생기는 산출물은 `.wiseman/wiseman.db` 단 하나**다(+ 선택적으로 `.wiseman/meta.json`). 서버 코드는 프로젝트에 복제되지 않는다.

```
대상 프로젝트/
└── .wiseman/
    ├── wiseman.db     ← 이 프로젝트의 지식 (SQLite, 단일 진실 소스)
...
→ summon-wiseman 스킬 + 범용 Wiseman 서버가 자동 설치/등록됨
→ 아직 어떤 프로젝트에도 db가 없으므로, 서버는 "현자가 아직 없습니다" 상태
```
...
→ 지식을 .wiseman/wiseman.db 로 빌드
→ 사용자에게 요약 보고 + 활성화 확인
→ 이후 코딩/다른 스킬에서 wiseman MCP 도구로 질의
```

서버는 매 질의마다 db를 **lazy하게 읽으므로**, db가 생기는 즉시 답하기 시작한다(서버 재시작 불필요). db가 없으면 "현자가 아직 없습니다. `/summon-wiseman` 실행하세요"를 반환한다.

---

## 4. `summon-wiseman` 스킬 (빌드 시점)
...
7. **DB 빌드** — 위 지식을 정제하여 `wiseman.db`에 기록
...
| 도구 | 역할 |
|------|------|
| `list_topics()` | 이 프로젝트 현자가 아는 라이브러리/토픽 목록 반환 (에이전트가 "무엇을 물을 수 있는지" 파악). Context7의 `resolve-library-id` 대응 |
| `ask_wiseman(query, topic?, library?)` | FTS5 BM25 검색. 관련 엔트리(제목·본문·**출처**·버전·confidence) 랭킹 반환. Context7의 `get-library-docs` 대응 |
...
- db 파일 없음 → 모든 도구가 "현자가 아직 없습니다. `/summon-wiseman`을 실행하세요" 안내
- db는 있으나 엔트리 0개 → "현자가 비어있습니다" 안내
...
```
[빌드: /summon-wiseman]
프로젝트 분석 → 병렬 조사(웹/코드) → 정제 → 품질 게이트 → wiseman.db 기록 → 사용자 보고

[질의: 코딩 중]
Claude → wiseman MCP (list_topics / ask_wiseman) → wiseman.db FTS5 검색 → 엔트리+출처 반환 → Claude가 활용
```
...
- Claude Code **플러그인 마켓플레이스**로 배포 (`/plugin install wiseman`)
...
- **서버 단위 테스트** — 알려진 시드 db에 대해 `list_topics`/`ask_wiseman`/`get_entry`가 기대 결과 반환, 빈/없는 db 처리
...
**포함:**
- `wiseman` 플러그인 구조 (plugin.json + .mcp.json)
- `summon-wiseman` 스킬 (기존/새 프로젝트 분기, 병렬 조사, 품질 게이트)
- 범용 Wiseman 서버 (Python + FastMCP, `uv run`)
- SQLite 스키마 + FTS5 인덱서
- 3개 도구: `list_topics`, `ask_wiseman`, `get_entry`
...
- 증분 재-summon
- 저장소 디렉토리 `teachers-mcp` → `wiseman` 리네임

---

## 13. 열린 질문 (구현 계획 전 확정 필요)

1. **재-summon 정책** — 두 번째 `/summon-wiseman` 실행 시: 전체 재빌드 vs 증분 갱신 vs 사용자 선택?
2. **조사 깊이/비용** — 라이브러리당 서브에이전트 몇 개까지? 토큰 예산 상한?
3. **핵심 라이브러리 선정** — 모든 의존성을 다 조사하면 비쌈. 직접 의존성만? 사용자 확인 후 선별?
4. **`.wiseman/` git 커밋 여부** — db를 팀과 공유(커밋) vs 개인별(gitignore)? 기본값 권장 필요.
5. **도구 이름** — `ask_wiseman` 등 네이밍이 에이전트의 호출 유도에 적합한지 (MCP 도구 설명문 튜닝).
